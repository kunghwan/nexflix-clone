...
const PDSample1 = () => {
  ...
  return (
    <div>
      ...
      />
      {/* 2. 컴포넌트를 불러오는 곳에서 정확한 속성을 입력해주세요. */}
...
const SampleInput = (props) => {
  // 3. 컴포넌트의 인자를 정할 때  js에서는 타입을 지정할수없음 그래서 좀 불편
...
// 내가 받아야할 인자값과 전달해야하는 값이 동일한 객체 형태일 때에만  {...값}으로 전달 받을 수있다.
// 내가 필요한 것을 꺼내오면 됨
const SampleOBJ = ({ first, last, middle }) => {
  //  객체를 받아올 떄는 {}가 아닌 그냥 ()안에다가 바로 인자를 선언해서 가져오면 됨
...
// Props-Drilling을 할 떄 리액트 컴포넌트 끼리 주고 받는 값은 객체다.
const Item = ({ text }) => {
  return <p>{text}</p>;
};
